  ...
  return (
    <div className="space-y-8">
            ...
            {pending.map((a) => (
              <div key={a.id} className="bg-white rounded-xl border border-orange-200 p-4 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">CUIT {a.cuit} · {a.user.email}</p>
                  <p className="text-xs text-gray-400">Registrada {new Date(a.user.createdAt).toLocaleDateString("es-AR")}</p>
                </div>
      ...
      <div>
        ...
        {approved.length === 0 ? (
          <p className="text-sm text-gray-400">Ninguna agencia aprobada aún.</p>
        ) : (
          <div className="space-y-2">
            {approved.map((a) => (
              <div key={a.id} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="font-medium text-gray-900 text-sm">{a.companyName}</p>
                  <p className="text-xs text-gray-400 mt-0.5">CUIT {a.cuit} · {a.user.email} · {a._count.properties} propiedad{a._count.properties !== 1 ? "es" : ""}</p>
                  {a.approvedAt && (
                    <p className="text-xs text-gray-400">Aprobada {new Date(a.approvedAt).toLocaleDateString("es-AR")}</p>
                  )}
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <form action={rejectAgency.bind(null, a.id)}>
                    <SubmitButton className="text-xs text-gray-400 hover:text-red-500 transition-colors" pendingText="...">
                      Revocar
                    </SubmitButton>
                  </form>
                </div>
