    ...
    <div className="space-y-8">
      ...
      {pending.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          <p className="text-sm text-gray-500">Sin documentos pendientes de revisión.</p>
        </div>
      ) : (
        <div>
            ...
            {pending.map((d) => (
              <div key={d.id} className="bg-white rounded-xl border border-red-200 p-4 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-600 mt-1">{new Date(d.createdAt).toLocaleDateString("es-AR")}</p>
                </div>
            ...
            {resolved.map((d) => (
              <div key={d.id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 opacity-70">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-600 mt-0.5 italic">&quot;{d.reason}&quot;</p>
                </div>
